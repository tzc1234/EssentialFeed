...
class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_loadFeedCompletion_dispatchesFeedFromBackgroundToMainThread() {
        let (sut, loader) = makeSUT()
        sut.loadViewIfNeeded()
    ...
    func test_loadMoreCompletion_dispatchesFeedFromBackgroundToMainThread() {
        let (sut, loader) = makeSUT()
        sut.loadViewIfNeeded()
        loader.completeFeedLoading(at: 0)
        sut.simulateLoadMoreFeedAction()
    ...
    func test_feedImageView_cancelsImageLoadingWhenNotVisibleAnymore() {
        ...
        XCTAssertEqual(loader.loadedImageURLs, [], "Expected no cancelled image URL requests until image is not visible")
        
        sut.simulateFeedImageViewNotVisible(at: 0)
        XCTAssertEqual(loader.cancelledImageURLs, [image0.url], "Expected one cancelled URL request once first image is not visible anymore")
    ...
    func test_feedImageViewLoadingIndicator_isVisibleWhileLoadingImage() {
        ...
        XCTAssertEqual(view0?.isShowingImageLoadingIndicator, false, "Expacted no loading indicator for first view once first image loading completes successfully")
        XCTAssertEqual(view1?.isShowingImageLoadingIndicator, true, "Expacted no loading indicator state change for second view once first image loading completes successfully")
        
        loader.completeImageLoadingWithError(at: 1)
        XCTAssertEqual(view0?.isShowingImageLoadingIndicator, false, "Expacted no loading indicator state change for first view once second image loading completes with error")
        XCTAssertEqual(view1?.isShowingImageLoadingIndicator, false, "Expacted no loading indicator for second view once second image loading completes with error")
    }
    ...
    func test_feedImageViewRetryButton_isVisibleOnImageURLLoadError() {
        ...
        XCTAssertEqual(view1?.isShowingRetryAction, false, "Expected no retry state change for second view once first image loading completes successfully")
    ...
    func test_feedImageView_configuresViewCorrectlyWhenTransitioningFromNearVisibleToVisibleWhileStillPreloadingImage() {
        ...
        XCTAssertEqual(view?.renderedImage, nil, "Expected no rendered image when view becomes visible while still preloading image")
        XCTAssertEqual(view?.isShowingRetryAction, false, "Expected no retry action when view becomes visible while still preloading image")
        XCTAssertEqual(view?.isShowingImageLoadingIndicator, true, "Expected loading indicator when view becomes visible while still preloading image")
        ...
        XCTAssertEqual(view?.isShowingRetryAction, false, "Expected no retry action after image preloads successfully")
    ...
    func test_loadFeedCompletion_rendersErrorMessageOnErrorUntilNextReload() {
        ...
        XCTAssertEqual(sut.errorMessage, loadError, "Expected an error message after received an error from feed loading request")
