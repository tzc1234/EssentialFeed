...
//  EssentialApp
//
//  Created by Tsz-Lung on 09/04/2023.
//
...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    private lazy var scheduler: AnyDispatchQueueScheduler = DispatchQueue(
        label: "com.essentialdevelop.infra.queue",
        qos: .userInitiated,
    ...
    private lazy var logger = Logger(subsystem: "com.essentialDevelop.EssentialAppCaseStudy", category: "main")
        ...
        handleEvents(
            receiveCompletion: { result in
                if case let .failure(error) = result {
                    logger.trace("Fail to load url: \(url) with \(error.localizedDescription)")
                }
            }).eraseToAnyPublisher()
    }
