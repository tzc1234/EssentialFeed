...
//
//  Created by Tsz-Lung on 26/12/2022.
//
...
final class EssentialFeedAPIEndToEndTests: XCTestCase {
    ...
    func test_endToEndTestServerGETFeedImageDataResult_matchedFixedTestAccountData() {
    ...
    private func getLoadFeedResult(file: StaticString = #filePath, line: UInt = #line) -> FeedLoader.Result? {
        let loader = RemoteLoader(url: feedTestServerURL, client: ephemeralClient(file: file, line: line), mapper: FeedItemsMapper.map)
    ...
    private func expectedImage(at index: Int) -> FeedImage {
        FeedImage(id: id(at: index), description: description(at: index), location: location(at: index), url: imageURL(at: index))
    }
